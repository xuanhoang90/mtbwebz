<?php
	...
	$CMS->skin_product_category = new Skin_ProductCategory();
	class Skin_ProductCategory{
		public function __construct(){
			return true;
		}
		public function PageDefault(){
			global $CMS, $DB;
			$CMS->admin['system']->LoadLanguage('admin_product_category');
			$custom_style = <<<CSS
			<!-- Theme style -->
		...
		public function QuickAction(){
			...
			$output =<<<HERE
				<div class="quickaccess">
					<a class="add action" href="{$CMS->vars['root_domain']}?site=admin&page=product_category&action=add"><i class="fa fa-plus"></i></a>
				</div>
HERE;
			return $output;
		}
											...
											<div class="box-header with-border">
												...
												<div class="btn-group pull-right">
													...
													<ul class="dropdown-menu">
														<li><a href="{$CMS->vars['root_domain']}?site=admin&page=product_category&action=viewlist&page_number={$currentPage}&item_per_page=10">10 {$CMS->vars['lang']['acp_object_itemperpage']}</a></li>
														<li><a href="{$CMS->vars['root_domain']}?site=admin&page=product_category&action=viewlist&page_number={$currentPage}&item_per_page=10">20 {$CMS->vars['lang']['acp_object_itemperpage']}</a></li>
														<li><a href="{$CMS->vars['root_domain']}?site=admin&page=product_category&action=viewlist&page_number={$currentPage}&item_per_page=10">30 {$CMS->vars['lang']['acp_object_itemperpage']}</a></li>
														<li><a href="{$CMS->vars['root_domain']}?site=admin&page=product_category&action=viewlist&page_number={$currentPage}&item_per_page=10">40 {$CMS->vars['lang']['acp_object_itemperpage']}</a></li>
														<li><a href="{$CMS->vars['root_domain']}?site=admin&page=product_category&action=viewlist&page_number={$currentPage}&item_per_page=10">50 {$CMS->vars['lang']['acp_object_itemperpage']}</a></li>
													</ul>
												</div>
											</div><!-- /.box-header -->
											<div class="box-body">
...
HERE;
														$data = $CMS->main_object->LoadObjectTypeListData("product_category", $currentPage, $ItemsPerPage);
														foreach($data as $object){
															...
															$output .=<<<HERE
																<tr>
																	...
																	<td>{$parent['name']}</td>
																	<td>
																		<a class="act edit" target="_blank" href="{$CMS->vars['root_domain']}?site=admin&page=product_category&action=edit&id={$object['object_id']}"><i class='fa fa-edit'></i> {$CMS->vars['lang']['acp_table_th_action_edit']}</a>
																		<a class="act preview" target="_blank" href="#"><i class='fa fa-eye'></i> {$CMS->vars['lang']['acp_table_th_action_preview']}</a>
																		<a class="act clone" target="_blank" href="{$CMS->vars['root_domain']}?site=admin&page=product_category&action=clone&id={$object['object_id']}"><i class='fa fa-clone'></i> {$CMS->vars['lang']['acp_table_th_action_clone']}</a>
																		<a class="act delete" target="_blank" href="{$CMS->vars['root_domain']}?site=admin&page=product_category&action=delete&id={$object['object_id']}"><i class='fa fa-trash'></i> {$CMS->vars['lang']['acp_table_th_action_delete']}</a>
																	</td>
																	<td>
...
HERE;
											...
											<div class="box-footer clearfix">
												...
												{$CMS->admin['skin_global']->ObjectPageNav("product_category", $currentPage, $ItemsPerPage)}
											</div>
										</div><!-- /.box -->
...
HERE;
					//Setup object data
					if($CMS->main_object->SetupObjectData("product_category", $CMS->input['id'])){
						$output .= $this->FormProductCategory();
					}else{
						//object not found
...
HERE;
		...
		public function ObjectNotFound(){
			global $CMS, $DB;
			$output = "";
			$form_title = $CMS->vars['lang']['acp_main_page_'.$CMS->input['action'].'_product_category'];
			$output =<<<HERE
					<!-- Content Wrapper. Contains page content -->
					<!-- Content Wrapper. Contains page content -->
						<div class="content-wrapper">
...
HERE;
		...
		public function FormProductCategory(){
			global $CMS, $DB;
			$output = "";
			if($CMS->input['id'] != ""){
			...
			$output =<<<HERE
					...
					<!-- Content Wrapper. Contains page content -->
						<div class="content-wrapper">
							...
							<!-- Main content -->
							<section class="content"><form class="form-horizontal" role="form" method="POST" action="{$CMS->vars['root_domain']}/?site=admin&page=product_category&action={$CMS->input['action']}_do{$exUrl}">
								<div class="row acp-table-padding-bottom">
...
HERE;
		...
		public function ObjectNameInput($langAcess = false){
			...
			$output = "";
			foreach($langAcess as $key=>$lgg){
				$lang = $CMS->admin['system']->SpecialLoadLanguage($key.'/admin_product_category');
				...
				$output .=<<<HERE
					<div class="x_obj_input x_obj_input_{$lgg['id']}">
						...
						<div class="col-md-9 col-md-offset-3">
							<p class="object_link_preview object_link_preview_{$lgg['id']}"><i class="fa fa-chrome"></i> {$lang['acp_object_editor_niceurl_real']}: <span>http://mtbweb.com/product_category/<i class="nice_url_preview" style="color: red;">{$val}</i></span></p>
						</div>
					</div>
HERE;
			}
...
HERE;
		...
		public function ObjectTagInput($langAcess = false){
			...
			foreach($langAcess as $key=>$lgg){
				...
				$output .=<<<HERE
					<div class="x_obj_input x_obj_input_{$lgg['id']}" data="{$CMS->vars['root_domain']}?site=admin&page=tags&action=ajax&subact=auto_complete&object_type=product_category">
