<?php
	...
	$CMS->skin_product = new Skin_Product();
	class Skin_Product{
		...
		public function PageDefault(){
			global $CMS, $DB;
			$CMS->admin['system']->LoadLanguage('admin_product');
			$custom_style = <<<CSS
			<!-- Theme style -->
		...
		public function QuickAction(){
			...
			$output =<<<HERE
				<div class="quickaccess">
					<a class="add action" href="{$CMS->vars['root_domain']}?site=admin&page=product&action=add"><i class="fa fa-plus"></i></a>
				</div>
HERE;
			return $output;
		}
											...
											<div class="box-header with-border">
												...
												<div class="btn-group pull-right">
													...
													<ul class="dropdown-menu">
														<li><a href="{$CMS->vars['root_domain']}?site=admin&page=product&action=viewlist&page_number={$currentPage}&item_per_page=10">10 {$CMS->vars['lang']['acp_object_itemperpage']}</a></li>
														<li><a href="{$CMS->vars['root_domain']}?site=admin&page=product&action=viewlist&page_number={$currentPage}&item_per_page=10">20 {$CMS->vars['lang']['acp_object_itemperpage']}</a></li>
														<li><a href="{$CMS->vars['root_domain']}?site=admin&page=product&action=viewlist&page_number={$currentPage}&item_per_page=10">30 {$CMS->vars['lang']['acp_object_itemperpage']}</a></li>
														<li><a href="{$CMS->vars['root_domain']}?site=admin&page=product&action=viewlist&page_number={$currentPage}&item_per_page=10">40 {$CMS->vars['lang']['acp_object_itemperpage']}</a></li>
														<li><a href="{$CMS->vars['root_domain']}?site=admin&page=product&action=viewlist&page_number={$currentPage}&item_per_page=10">50 {$CMS->vars['lang']['acp_object_itemperpage']}</a></li>
													</ul>
												</div>
											</div><!-- /.box-header -->
											<div class="box-body">
...
HERE;
														$data = $CMS->main_object->LoadObjectTypeListData("product", $currentPage, $ItemsPerPage);
														foreach($data as $object){
															...
															$output .=<<<HERE
																<tr>
																	...
																	<td>{$parent['name']}</td>
																	<td>
																		<a class="act edit" target="_blank" href="{$CMS->vars['root_domain']}?site=admin&page=product&action=edit&id={$object['object_id']}"><i class='fa fa-edit'></i> {$CMS->vars['lang']['acp_table_th_action_edit']}</a>
																		<a class="act preview" target="_blank" href="#"><i class='fa fa-eye'></i> {$CMS->vars['lang']['acp_table_th_action_preview']}</a>
																		<a class="act clone" target="_blank" href="{$CMS->vars['root_domain']}?site=admin&page=product&action=clone&id={$object['object_id']}"><i class='fa fa-clone'></i> {$CMS->vars['lang']['acp_table_th_action_clone']}</a>
																		<a class="act delete" target="_blank" href="{$CMS->vars['root_domain']}?site=admin&page=product&action=delete&id={$object['object_id']}"><i class='fa fa-trash'></i> {$CMS->vars['lang']['acp_table_th_action_delete']}</a>
																	</td>
																	<td>
...
HERE;
											...
											<div class="box-footer clearfix">
												...
												{$CMS->admin['skin_global']->ObjectPageNav("product", $currentPage, $ItemsPerPage)}
											</div>
										</div><!-- /.box -->
...
HERE;
					//Setup object data
					if($CMS->main_object->SetupObjectData("product", $CMS->input['id'])){
						$output .= $this->FormProduct();
					}else{
						//object not found
...
HERE;
		...
		public function ObjectNotFound(){
			global $CMS, $DB;
			$output = "";
			$form_title = $CMS->vars['lang']['acp_main_page_'.$CMS->input['action'].'_product'];
			$output =<<<HERE
					<!-- Content Wrapper. Contains page content -->
					<!-- Content Wrapper. Contains page content -->
						<div class="content-wrapper">
			...
			$output =<<<HERE
					...
					<!-- Content Wrapper. Contains page content -->
						<div class="content-wrapper">
							...
							<!-- Main content -->
							<section class="content"><form class="form-horizontal" role="form" method="POST" action="{$CMS->vars['root_domain']}/?site=admin&page=product&action={$CMS->input['action']}_do{$exUrl}">
								<div class="row acp-table-padding-bottom">
									<div class="col-md-9 col-sm-12 col-xs-12">
										...
										<div class="box">
											...
											<div class="box-body">
												...
												<div class="form-group">
													{$this->ObjectPrimaryPriceInput($langAcess)}
												</div>
												
												<div class="form-group">
													{$this->ObjectSKUInput($langAcess)}
												</div>
												
												<div class="form-group">
													{$this->ObjectAttachmentInput($langAcess)}
												</div>
												
												<div class="form-group">
													{$this->ObjectIconInput($langAcess)}
												</div>
												
												<div class="form-group">
													{$this->ObjectExtendInput($langAcess)}
												</div>
...
HERE;
		...
		public function ObjectNameInput($langAcess = false){
			...
			$output = "";
			foreach($langAcess as $key=>$lgg){
				$lang = $CMS->admin['system']->SpecialLoadLanguage($key.'/admin_product');
				...
				$output .=<<<HERE
					<div class="x_obj_input x_obj_input_{$lgg['id']}">
						...
						<div class="col-md-9 col-md-offset-3">
							<p class="object_link_preview object_link_preview_{$lgg['id']}"><i class="fa fa-chrome"></i> {$lang['acp_object_editor_niceurl_real']}: <span>http://mtbweb.com/product/<i class="nice_url_preview" style="color: red;">{$val}</i></span></p>
						</div>
					</div>
HERE;
			}
...
HERE;
		...
		public function CreateListCategoryInSelect($lang = "1", $parent = 0, $type = "product_category", $tab = ""){
			global $CMS, $DB, $OBJECT;
			$output = "";
			//$tab = ":_";
			$data = $CMS->main_object->LoadObjectDataByOption($parent, $lang, $type = "product_category");
			if($data){
				foreach($data as $cat){
					...
					if($cat['parent'] == "0"){
						...
						<optgroup>
							...
							{$this->CreateListCategoryInSelect($lang, $cat['id'], "product_category", $tab.":_")}
						</optgroup>
HERE;
					}else{
						$output .=<<<HERE
...
HERE;
		...
		public function ObjectTagInput($langAcess = false){
			...
			foreach($langAcess as $key=>$lgg){
				...
				$output .=<<<HERE
					<div class="x_obj_input x_obj_input_{$lgg['id']}" data="{$CMS->vars['root_domain']}?site=admin&page=tags&action=ajax&subact=auto_complete&object_type=product">
...
HERE;
		...
		public function ObjectPrimaryPriceInput($langAcess = false){
			global $CMS, $DB, $OBJECT;
			$output = "";
			foreach($langAcess as $key=>$lgg){
				$lang = $CMS->admin['system']->SpecialLoadLanguage($key.'/admin_product');
				if($OBJECT[$lgg['id']]['primary_price'] != ""){
					$val = $OBJECT[$lgg['id']]['primary_price'];
				}else{
					$val = "";
				}
				$output .=<<<HERE
					<div class="x_obj_input x_obj_input_{$lgg['id']}">
						<label class="control-label col-md-3" for="object_primary_price">Price:</label>
						<div class="col-md-9">
							<input type="text" class="form-control object_data_primary_price object_data_{$lgg['id']}_primary_price" name="object_data[{$lgg['id']}][primary_price]" id="object_data_{$lgg['id']}_primary_price" placeholder="" value="{$val}" />
						</div>
					</div>
HERE;
			}
		...
		public function ObjectSKUInput($langAcess = false){
			global $CMS, $DB, $OBJECT;
			$output = "";
			foreach($langAcess as $key=>$lgg){
				$lang = $CMS->admin['system']->SpecialLoadLanguage($key.'/admin_product');
				if($OBJECT[$lgg['id']]['product_code'] != ""){
					$val = $OBJECT[$lgg['id']]['product_code'];
				}else{
					$val = "";
				}
				$output .=<<<HERE
					<div class="x_obj_input x_obj_input_{$lgg['id']}">
						<label class="control-label col-md-3" for="object_product_code">SKU:</label>
						<div class="col-md-9">
							<input type="text" class="form-control object_data_product_code object_data_{$lgg['id']}_product_code" name="object_data[{$lgg['id']}][product_code]" id="object_data_{$lgg['id']}_product_code" placeholder="" value="{$val}" />
						</div>
					</div>
HERE;
			}
		...
		public function ObjectAttachmentInput($langAcess = false){
			global $CMS, $DB, $OBJECT;
			$output = "";
			foreach($langAcess as $key=>$lgg){
				$lang = $CMS->admin['system']->SpecialLoadLanguage($key.'/admin_product');
				if($OBJECT[$lgg['id']]['attachment_list'] != ""){
					$val = $OBJECT[$lgg['id']]['attachment_list'];
				}else{
					$val = "";
				}
				$output .=<<<HERE
					<div class="x_obj_input x_obj_input_{$lgg['id']}">
						<label class="control-label col-md-3" for="object_attachment_list">Album:</label>
						<div class="col-md-9">
							<input type="text" class="form-control object_data_attachment_list object_data_{$lgg['id']}_attachment_list" name="object_data[{$lgg['id']}][attachment_list]" id="object_data_{$lgg['id']}_attachment_list" placeholder="" value="{$val}" />
						</div>
					</div>
HERE;
			}
		...
		public function ObjectIconInput($langAcess = false){
			global $CMS, $DB, $OBJECT;
			$output = "";
			foreach($langAcess as $key=>$lgg){
				$lang = $CMS->admin['system']->SpecialLoadLanguage($key.'/admin_product');
				if($OBJECT[$lgg['id']]['product_icon'] != ""){
					$val = $OBJECT[$lgg['id']]['product_icon'];
				}else{
					$val = "";
				}
				$output .=<<<HERE
					<div class="x_obj_input x_obj_input_{$lgg['id']}">
						<label class="control-label col-md-3" for="object_product_code">Icon:</label>
						<div class="col-md-9">
							<input type="text" class="form-control object_data_product_icon object_data_{$lgg['id']}_product_icon" name="object_data[{$lgg['id']}][product_icon]" id="object_data_{$lgg['id']}_product_icon" placeholder="" value="{$val}" />
						</div>
					</div>
HERE;
			}
		...
		public function ObjectExtendInput($langAcess = false){
			global $CMS, $DB, $OBJECT;
			$output = "";
			foreach($langAcess as $key=>$lgg){
				$lang = $CMS->admin['system']->SpecialLoadLanguage($key.'/admin_product');
				if($OBJECT[$lgg['id']]['extend_properties'] != ""){
					$val = $OBJECT[$lgg['id']]['extend_properties'];
				}else{
					$val = "";
				}
				$output .=<<<HERE
					<div class="x_obj_input x_obj_input_{$lgg['id']}">
						<label class="control-label col-md-3" for="object_extend_properties">Extend properties:</label>
						<div class="col-md-9">
							<input type="text" class="form-control object_data_extend_properties object_data_{$lgg['id']}_extend_properties" name="object_data[{$lgg['id']}][extend_properties]" id="object_data_{$lgg['id']}_extend_properties" placeholder="" value="{$val}" />
						</div>
					</div>
HERE;
			}
